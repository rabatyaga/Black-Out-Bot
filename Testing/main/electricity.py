...
def get_time_zone(time=None):
    send_log('Started First Button Check' + '=' * 100)
    send_log(f"Got time: {time}")
    if time is None:
    ...
    if time >= 21 or time == 0:
        ...
        send_log(f"Get Time Zone Function: time_zone: {time_zone}, time: {time}")
        return time_zone
...
def get_condition(group: int) -> str:
    ...
    send_log(f"Get Condition Function: day: {day}, time_zone: {time_zone}, group: {group}")
    if group == 1:
